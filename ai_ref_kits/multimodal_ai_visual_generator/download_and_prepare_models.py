...
def prepare_llm_model():
    if not Path(model_dir+"/tinyllama-1.1b-chat-v1.0").exists():
        print("TinyLlama Model not downloaded.")
        cmd = "optimum-cli export openvino --model TinyLlama/TinyLlama-1.1B-Chat-v1.0 --task text-generation-with-past --weight-format int4 --group-size -1 --ratio 1.0 --sym "  + model_dir + "/tinyllama-1.1b-chat-v1.0/INT4_compressed_weights"
        print("llm download command:",cmd)
        os.system(cmd)
    else:
        print("TinyLlama Model already downloaded.")
    ...
    if not Path(model_dir+"/depth_anything_v2_vits.xml").exists():
        try:
            ...
            if "libGL.so.1" in str(e):
                print("Warning: OpenCV dependencies not available. Skipping Depth-Anything-V2 model preparation.")
                print("To install the required dependencies, run: apt-get update && apt-get install -y libgl1-mesa-glx")
            else:
                print(f"Error preparing Depth-Anything-V2 model: {e}")
    else:
